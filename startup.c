...
/* These are all of the handlers(functions that will be located within the vector table
 * SIDENOTE: __attribute__((weak, alias("Default_Handler")) creates an alias of 
 * Default_Handler and can be overwritten if another function is created within
 * the same ELF later on*/
...
/*
 * These are the boundaries for each section created by the linker(extern just tells 
 * the compiler that the variable is located outside the scope of the 
 * object
 * */
...
/*
 * Initialization of the vector table which will start at 0x00000000
 */             

__attribute__ ((weak,section(".isr_vector")))
void(*nvictable[])(void) = {
    (void*)STACK_START,             /* 0 = SP = endereco de stack_top */
...
void Default_Handler(void) {
    ...
    intmask = *((uint32_t *) (0xE000ED00U+0xD04U));
    while(1) {} /* Loop */
    /* NEVER */
}
...
void __attribute__((weak,naked)) Reset_Handler(void) {
    ...
    /* This is used to move the data from ROM to RAM*/
    pSource = &_etext;
    pDest   = &_data;
    while( pDest < &_edata ) {
        *pDest++ = *pSource++;
